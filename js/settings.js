 ...
 *
 * 【调用链】
 *   入口：app.js 在 DOMContentLoaded 时调用 bindSettingsEvents() 绑定 UI 事件，
...
(function() {
   ...
   *   5 秒 TTL 缓存能显著减少 JSON.parse 和磁盘读取开销，同时保证设置变更后最多 5 秒生效。
   */
   ...
   *   safeSetItem（定义在 app.js）对 localStorage 不可用场景做了 try/catch 包装，
