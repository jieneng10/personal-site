...
if (!sb) { showToast(tSync('auth.serviceDown'), 'warn'); return false; }
showLoading(tSync('auth.loginLoading'));
var result = await sb.auth.signInWithPassword({ email: email, password: password });
hideLoading();
if (result.error) {
  var msg = result.error.message === 'Invalid login credentials' ? tSync('auth.loginFailed') : result.error.message;
...
showToast(tSync('auth.loginSuccess'), 'success');
...
if (!sb) { showToast(tSync('auth.serviceDown'), 'warn'); return false; }
showLoading(tSync('auth.registerLoading'));
...
showToast(tSync('auth.registerSuccess'), 'success');
return true;
}
...
if (confirm(tSync('settings.resetConfirm'))) {
  ...
  showToast(tSync('settings.resetDone'), 'success');
}
}
...
if (sessionResult.data.session) {
  if (confirm(tSync('auth.logoutConfirm'))) {
    await sbLogout();
    location.reload();
  }
} else {
...
if (btnLogin) {
  btnLogin.addEventListener('click', async function() {
    ...
    if (!email || !password) {
      var errEl = document.getElementById('loginError');
      if (errEl) errEl.textContent = tSync('auth.fillEmailPassword');
      return;
    }
...
if (btnChangePwd) {
  btnChangePwd.addEventListener('click', async function() {
    ...
    if (!oldPwd) {
      showToast(tSync('auth.oldPasswordRequired'), 'warn');
      return;
    }
    ...
    if (signInResult.error) {
      hideLoading();
      showToast(tSync('auth.oldPasswordWrong'), 'error');
      return;
    }
    ...
    if (result.error) {
      ...
      showToast(tSync('auth.passwordUpdated'), 'success');
    }
  });
}
