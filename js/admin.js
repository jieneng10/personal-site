 ...
 *
 * 【依赖的 window 全局变量】
 *   window.sb                  — Supabase 客户端实例（由 supabase.js 注入）
 *   window.showToast(msg,type) — 全局 toast 提示（由 common.js 提供）
 *   window.escHtml(s)          — HTML 转义函数（由 common.js 提供）
 *   window.sanitizeHtml(html)  — HTML 净化函数（由 common.js 提供）
 *   window.EventBus            — 事件总线（用于跨模块通信）
 *   window._isLoggedIn         — 当前是否已登录（由 auth.js 维护）
 *   window.DEFAULT_WALLPAPERS  — 内置默认壁纸列表（由 supabase.js 提供）
 *   window.DEFAULT_BGMS        — 内置默认 BGM 列表（由 supabase.js 提供）
 ...
 *
 * 【为什么用 IIFE 包裹】
 *   避免污染全局命名空间。模块内部变量（editingId、_previewBound 等）
 *   对外不可见，只有显式挂到 window 上的函数才可被外部调用。
 */
(function() {
   ...
   * sb — Supabase 客户端引用
   *
   * 【来源】 window.sb，由 supabase.js 在页面加载时初始化。
   ...
   * toast — 本地 toast 通知函数
   *
   * 【来源】 优先使用 window.showToast（common.js），若未加载则用 DOM 操作降级。
   ...
   * esc — 本地 HTML 转义函数
   *
   * 【来源】 优先使用 window.escHtml（common.js），若未加载则用字符串替换降级。
   ...
   * 【为什么暴露到 window】 admin.js 是 IIFE 封装的，外部只能通过 window 访问。
   */
