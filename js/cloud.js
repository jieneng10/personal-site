 ...
 *     window.showLoading     — 加载中遮罩 (由 utils.js 注入)
 *     window.hideLoading     — 隐藏加载遮罩 (由 utils.js 注入)
 *     window.getCachedUser   — 获取当前用户信息 (由 auth.js 注入)
 *     window.sbStoragePath   — 生成 Storage 路径 (由 supabase.js 注入)
 *     window.sbUpload        — 上传文件到 Storage (由 supabase.js 注入)
 *     window.sbSignedUrl     — 生成 Signed URL (由 supabase.js 注入)
 *     window.sbDelete        — 删除 Storage 文件 (由 supabase.js 注入)
 *
 *   写入 (暴露给外部):
 ...
 *   - 调用 window.showToast / showLoading / hideLoading
 */
...
(function() {
  ...
  /**
   * escHtml() — 局部定义，转义 HTML 特殊字符
   *
   * 【为什么局部定义而不是复用 window.escHtml】
   *   1. cloud.js 可能在 utils.js 之前加载
   *   2. 不依赖加载顺序，更健壮
   ...
   *
   * 【为什么暴露到 window】
   *   函数在 IIFE 中，外部不可见。main.js 初始化时调用 window.bindCloudEvents()。
   *
   * 【输入】无
   * 【输出】无
