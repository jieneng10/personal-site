 ...
 * 【与 window 全局变量的关系】
 *   读取:
 *     window.sb                    — Supabase client 实例 (由 supabase.js 注入)
 *     window.escHtml               — HTML 转义函数 (由 utils.js 注入)
 *     window.sanitizeHtml          — HTML 净化函数 (由 utils.js 注入)
 *     window.showToast             — Toast 通知函数 (由 utils.js 注入)
 ...
 *     window.safeSetItem           — 安全的 localStorage.setItem (由 utils.js 注入)
...
(function() {
   ...
   *   safeSetItem 是 utils.js 提供的包装函数，在 localStorage 配额满时
  ...
  async function fetchSupabaseNews() {
    if (!window.sb) return null; // Supabase client 未初始化 (如未加载 supabase.js)
    try {
  ...
  async function deleteNewsItem(id, newsDate) {
    if (!id) {
      ...
      if (typeof window.showToast === 'function') window.showToast('此资讯需在管理面板中删除', 'warn');
      return;
    }
    if (!confirm('确定删除这条资讯？')) return;
    if (!window.sb) {
      if (typeof window.showToast === 'function') window.showToast('服务不可用，请稍后重试');
      return;
    }
    try {
      var r = await window.sb.from('anime_news').delete().eq('id', id);
      if (r.error) {
        if (typeof window.showToast === 'function') window.showToast('删除失败: ' + r.error.message);
        return;
      }
   ...
   *
   * 【为什么优先用 window.escHtml】
   *   utils.js 中的 window.escHtml 可能有更完善的实现。
   *   如果没有加载 utils.js，fallback 到基本正则替换。
   *
   * 【输入】str — 任意字符串
   * 【输出】string — HTML 安全的字符串
   * 【调用者】renderNewsPanel(), openNewsDetail()
   */
