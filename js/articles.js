...
//   - 调用 window.showToast / window.escHtml / window.safeSetItem（来自 utils.js）
...
//
// 【为什么用 IIFE】
//   避免污染全局作用域。内部变量 activeFilter、searchQuery、articleView、
//   allTags、articles、_articleMap 对外不可见，只有通过 window 导出的 API 才能访问/修改。
//
(function() {
  // =========================================================================
  // 模块内部状态（闭包私有）
   ...
   *
   * 【输入】
   *   无参数。依赖全局 window.sb、window._isLoggedIn、data/articles.json。
   *
   * 【输出】
   ...
   *
   * 【它做什么】
   *   用 window.createCache 包装 _fetchArticleData，缓存 5 分钟。
