 ...
 *
 * 【它是什么】
 *   整个站点的"开机脚本"。在页面所有 <script> 标签加载完毕后，由自身 IIFE 执行。
 ...
 *
 * 【数据流向】
 *   window.sb (Supabase 客户端)  →  init() 检查会话
 *                                 →  若已登录：显示管理 UI、同步设置
 ...
 *
 * 【依赖的 window 全局变量（由其他 JS 文件注入）】
 *   window.sb                       — Supabase 客户端（supabase.js）
 *   window.getCachedUser()          — 获取缓存用户（auth.js）
 *   window.showLoading / hideLoading — 加载动画（common.js）
 *   window.showToast(msg,type)      — toast 通知（common.js）
 *   window.escHtml(s)               — HTML 转义（common.js）
 ...
 * 【为什么用 IIFE 包裹】
 *   与其他模块一致：避免污染全局命名空间。
 *   内部变量 _inited 只在模块内可见。
 */
(function() {

  // =========================================================================
  // 模块级变量 — 缓存从 window 读取的引用
  // =========================================================================

  /**
   * 缓存 window 上跨模块函数/对象的引用。
   *
   * 【为什么在 IIFE 开头缓存而不是每次用 window.xxx】
   *   1. 性能：局部变量访问比 window 属性查找略快
   *   2. 明确依赖：一眼看出本模块依赖哪些外部 API
   *   3. 注意：这里是「值复制」。如果 window 上的引用被替换（极少发生），
   *      缓存的值不会更新。对于本场景，各模块在页面加载时就挂载到 window
   *      且不会更改，所以缓存是安全的。
   */
  var sb = window.sb;
   ...
   *
   * 【为什么需要这个守卫】
   *   虽然 init() 目前只被调用一次（IIFE 末尾），但如果将来
